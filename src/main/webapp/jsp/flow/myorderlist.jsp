							...
							<c:forEach items="${orderList}" var="order" varStatus="varstatus">
								...
								<tbody style="margin-top: 10px;">
									...
									<c:forEach items="${order.oItem}" var="oit" varStatus="vs">
										<tr>
											<td>
												<img class="img-responsive col-md-4" style="height: 120px;" src="${oit.flow.upLoadImg.imgSrc}" alt="" />
												<span class="col-md-8">${oit.flow.flowName}</span>
											</td>
											<td>￥${oit.flow.price}</td>
											<td>${oit.quantity}</td>
											<c:choose>
												<c:when test="${vs.first}">
													...
													<td style="border:0;border-left:1px solid #ddd;">
														...
														<c:if test="${order.orderStatus eq 2}">
															<p>已发货（配送员：${order.chargername},联系方式:${order.chargerphone}）</p>
