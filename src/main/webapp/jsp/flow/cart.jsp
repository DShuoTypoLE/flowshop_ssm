...
<script type="text/javascript" src="js/flow/user_reg_login.js"></script>
<style type="text/css">
.wrapper {
	minheight: 400px;
}
					...
					<table class="table">
						...
						<c:choose>
							<c:when test="${!empty shopCart}">
								<tr class="info row">
									<th class="col-md-6" style="background:#fff;" >鲜花</th>
									...
									<tr class="row" id="pro-tr-${i.key}">
										<td><img class="img-responsive col-md-4"
											...
											<div class="col-md-8" style="padding:50px 0 50px 0;">
												...
												<%-- <p>${i.value.flow.author}</p>
												<p>${i.value.flow.press}</p> --%>
											</div>
										</td>
										<td style="padding:50px 0 50px 0;">￥<i>${i.value.flow.price}</i></td>
										...
										<img class="icon" src="images/flow/close-icon.png" alt="" style="margin:50px 0 50px 0;width:15px"></a></td>
									</tr>
								</c:forEach>
								<tr class="row">
							...
							<c:otherwise>
								<tr>
									<td colspan="6"
										...
										购物车内暂时没有鲜花， <a style="font-size: 24px; color: red"
										href="FlowList">去购物>></a>
									</td>
								</tr>
							</c:otherwise>
	  ...
	  //输入框处理
		...
		$("#tosettle").click(function(){
			$.get("UserServlet?action=landstatus",function(data){
				if(data.status=="y"){
					window.location.href="${basePath}jsp/flow/conorder.jsp";
				}else{
					$("#myModal").modal("show");
				}
			},"json")
