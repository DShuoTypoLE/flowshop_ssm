...
<body>
	...
	<h2 class="text-center">鲜花列表</h2>
	<div class="container-fulid content">
		<div class="funbtn">
			<a id="batDel" class="btn btn-danger" href="javascript:void(0)" >批量删除</a>
			<a class="btn btn-info" href="jsp/admin/FlowManageServlet?action=addReq">增加鲜花</a>
		...
		<table class="table table-striped table-hover">
			...
			<c:choose>
				<c:when test="${!empty flowList}">
					<c:forEach items="${flowList}" var="i" varStatus="n">
						<tr>
							<td class="noClick"><input type="checkbox" name="choice" value="${i.flowId}">
							<td>${i.flowName}</td>
							<td>${i.catalog.catalogName}</td>
							
							<td>${i.price}</td>
							<td class="noClick">
								<a class="btn btn-default btn-xs" href="jsp/admin/FlowManageServlet?action=detail&id=${i.flowId}">详情</a>
								<a class="btn btn-info btn-xs" href="jsp/admin/FlowManageServlet?action=edit&id=${i.flowId}">修改</a>
								<a class="btn btn-danger btn-xs" href="jsp/admin/FlowManageServlet?action=del&id=${i.flowId}" onclick="javascript:return confirm('确定要删除吗？');">删除</a>
							</td>
						</tr>
					</c:forEach>
				</c:when>
				<c:otherwise>
					<tr>
						<td colspan="8"><h4 class="text-center">当前无更多鲜花信息</h4></td>
					</tr>
				</c:otherwise>
			</c:choose>
		</table>
...
<script type="text/javascript">
	...
	$(".homePage").click(function(){
		window.location="${bsePath}jsp/admin/FlowManageServlet?action=list&page=1";
	})
	$(".prePage").click(function(){
		window.location="${basePath}jsp/admin/FlowManageServlet?action=list&page=${pageBean.prePage}";
	})
	$(".nextPage").click(function(){
		
		window.location="${basePath}jsp/admin/FlowManageServlet?action=list&page=${pageBean.nextPage}";
	})
	$(".lastPage").click(function(){
		
		window.location="${basePath}jsp/admin/FlowManageServlet?action=list&page=${pageBean.pageCount}";
	})
	//控制页面跳转范围
	$(".page-go").click(function(){
		...
		if(page > pageCount || page < 1 ){
			...
			window.location="${basePath}jsp/admin/FlowManageServlet?action=list&page="+page;
		}
	})
	...
	$("#batDel").click(function(){
		...
		if(confirm("你确定要删除"+choices.length+"条用户吗？")){
			...
			window.location = "${basePath}jsp/admin/FlowManageServlet?action=batDel&ids="+ids;
