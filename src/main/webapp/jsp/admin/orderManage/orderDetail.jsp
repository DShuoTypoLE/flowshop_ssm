...
<body>
	...
	<div class="container">
		...
		<div class="row">
			<div class="col-md-2 text-right">收货人</div>
			<div class="col-md-10">${order.shpeople}</div>
		</div>
		<div class="row">
			<div class="col-md-2 text-right">收货人联系方式</div>
			<div class="col-md-10">${order.shphone}</div>
		</div>
		<div class="row">
			<div class="col-md-2 text-right">收货地址</div>
			<div class="col-md-10">${order.shaddress}</div>
		</div>
		<div class="row">
			<div class="col-md-2 text-right">配送员</div>
			<div class="col-md-10">${order.chargername}</div>
		</div>
		<div class="row">
			<div class="col-md-2 text-right">配送员联系方式</div>
			<div class="col-md-10">${order.chargerphone}</div>
		</div>
		<div class="row">
		...
		<div class="row">
			<div class="col-md-12 text-left">订单鲜花信息</div>
			<div class="col-md-12">
					...
					<c:forEach items="${order.oItem}" var="i" varStatus="vs">
						<tr class="pro-list">
							<td><img width="50px" class="img-responsive col-md-2"
								src="${i.flow.upLoadImg.imgSrc }" alt="" />
								<div class="col-md-8">
									<p>${i.flow.flowName}</p>
									
								</div>
							</td>
							<td>￥<i>${i.flow.price}</i></td>
							<td>${i.quantity}</td>
							<c:choose>
