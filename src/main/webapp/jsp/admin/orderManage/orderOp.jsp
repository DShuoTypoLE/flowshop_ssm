				...
				<c:when  test="${!empty orderList}">
					<c:forEach items="${orderList}" var="i">
						<tr>
							...
							<td>${i.orderDate}</td>
							<td>${i.money}</td>
							<td>${i.chargername==null || i.chargername==''?'无':i.chargername}</td>
							<td>
							...
							<td>
								...
								<a class="btn btn-info btn-sm" href="jsp/admin/OrderManageServlet?action=ship&id=${i.orderId}" onclick="return isfh(${i.chargername})">发货</a>
								<a class="btn btn-default btn-sm" href="jsp/admin/OrderManageServlet?action=setCharger&id=${i.orderId}">设置配送员</a>
							</td>
						</tr>
					</c:forEach>
				</c:when>
...
<script type="text/javascript">
	function isfh(chargername){
		if(chargername==null || chargername==''){
			alert("请选择配送员进行发货操作")
			return false
		}else{
			return confirm('确定要发货了么！')
		}
	}
	//按钮禁用限制
